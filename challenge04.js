...
function reformarNombres(NombresArchivos, verfFileNumber) {
    ...
    let conteoFinal = 0
    archivo.map(elemento => {
        
        let conteoNewA;
        let conteoNewB;
        let conteoA = []
        let conteoB = []
        const [name, verf] = elemento.split("-")
        
        for (let i = 0; i < name.length; i++) {
            for (let j = 0; j < verf.length; j++) {

                if (name[i] == verf[j]) {
                    conteoA.push(name[i])
                    conteoB.push(verf[j])
                    let conteoVerf = verf.length
                    if (conteoVerf == conteoB.length){
                        conteoNewA = conteoA.join('')
