...
function analisis(data) {
    ...
    for (let i = 0; i < datos.length; i += 5) {
        let boolean = true;
        const [id, username, email, age, city]  = datos.slice(i, i + 5)
        ...
        if (ageRegex.test(age) || age !== undefined) {
        ...
        if (boolean === false && username[0] !== undefined) {
            massageHidden += username[0]
    ...
    console.log(massageHidden)
    return massageHidden
}
