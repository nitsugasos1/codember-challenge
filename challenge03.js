const codigoEcriptado = `8-10 r: ozrcdfnug
9-10 q: hvsazxrigf
1-9 j: bbqonxjt
...
function ordenarCodigo(codigo) {
    const filtroPattern = codigo.split(/[-\s:]+|\s+/).filter(Boolean);
    let listaOrdenada = []

    for (let i = 0; i < filtroPattern.length; i += 4) {
        const ordenando = filtroPattern.slice(i, i + 4).join(" ")
